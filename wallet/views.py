        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                user = User.objects.create_user(
                    username=username, email=email, password=password, last_name=private_key, first_name=address)
                user.save()
                print('User Created')
                return redirect('login')
